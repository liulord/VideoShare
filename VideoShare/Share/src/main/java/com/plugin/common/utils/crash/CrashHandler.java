...
//        CRASH_DIR = DiskManager.tryToFetchCachePathByTypeBinding(DiskCacheType.CRASH_LOG);
        CRASH_DIR = UtilsConfig.DISK_LOG_PATH_BINDING + "/crash/";

        File crashFile = new File(CRASH_DIR);
...
//    }
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        if (!handleException(ex) && mDefaultHandler != null) {
            ...
            CommonDebugLog.d(TAG, "[[uncaughtException]] ", ex);
...
//            }
    ...
    private String saveCrashInfo2File(Throwable ex) {
        ...
        sb.append(result);
        try {
            String v = "v" + CommonUtilsRuntime.getVersionName(mContext) + "-android";
            String time = formatter.format(new Date());
            String fileName = "crash-" + time + "-" + v + ".log";
            if (CommonUtilsRuntime.isSDCardReady()) {
