...
public class DiskManager {
	...
	private static final boolean DEBUG = false && CommonUtilsConfig.UTILS_DEBUG;

	private static String DISK_DIR = null;

	private static String DISK_TMP_DIR = null;

	private static final long MAX_DISK_SIZE = ((long) 50) * 1024 * 1024; // 100M
																			// ,
																			// 20M
																			// for
																			// debug
	private static final long MAX_TMP_DISK_SIZE = ((long) 20) * 1024 * 1024; // 100M
																				// ,
																				// 20M
																				// for
																				// debug
	...
	public static enum DiskCacheType {
		INPUTSTREAM_BIG_FILE_CACHE, PICTURE, DOWNLOAD_AUDIO, CRASH_LOG, CRASH_DEBUG_LOG, AUDIO_RECORD, MQTT_TRACE, AUDIO_EFFECTS, TMP, BASE, DOWNLOADED_APK
	}

	public static void init() {
		DISK_DIR = CommonUtilsConfig.DISK_DIR_PATH_BINDING;
		DISK_TMP_DIR = CommonUtilsConfig.DISK_TMP_DIR_PATH_BINDING;
	}
	...
	public static String tryToFetchCachePathByTypeBinding(DiskCacheType type) {
		...
		case DOWNLOAD_AUDIO:
			...
			if (FileUtil.isSDCardReady()) {
				...
				retDir = CommonUtilsConfig.DEVICE_INFO.flashDataPath;
			}
			break;
		case CRASH_LOG:
			retDir = retDir + "crash/";
		...
		case DOWNLOADED_APK:
			retDir = retDir
					+ (CommonUtilsRuntime.isExternalStorageAvailable() ? "downloaded_files/"
							: "/");
			break;
		case TMP:
	...
	public static LinkedList<FileInfo> collectCrashLogs() {
		if (!CommonUtilsConfig.RELEASE_UPLOAD_CRASH_LOG) {
			return null;
		}
		...
		if (DEBUG && files != null) {
			CommonUtilsConfig
					.LOGD("=================== beign list tryToUploadCrash ==================");
			for (FileInfo info : files) {
				CommonUtilsConfig.LOGD("        info : " + info.toString());
			}
			CommonUtilsConfig
					.LOGD("=================== end list tryToUploadCrash ==================");
		}
		...
		if (files != null && files.size() > 0) {
			...
			if (DEBUG) {
				CommonUtilsConfig
						.LOGD("=================== beign tryToUploadCrash after sort file ==================");
				for (FileInfo info : files) {
					CommonUtilsConfig.LOGD("        info : " + info.toString());
				}
				CommonUtilsConfig
						.LOGD("=================== beign tryToUploadCrash end sort file ==================");
			}
		}
	...
	// try {
	// SYSUploadCrashLogRequest request = new
	// SYSUploadCrashLogRequest.Builder(upFileInfo.fileName,
	...
	// for (FileInfo info : files) {
	// SYSUploadCrashLogRequest request = new
	// SYSUploadCrashLogRequest.Builder(info.fileName,
	...
	private static void tryToCleanDiskUnderDir(String fullPath, long maxSize) {
		CommonUtilsConfig.LOGD("[[tryToCleanDiskUnderDir]] try to clean path : "
                + fullPath + " }}}}}}}} =================");
		...
		if (!TextUtils.isEmpty(fullPath)) {
			...
			if (cleanDir.exists()) {
				...
				if (DEBUG) {
					CommonUtilsConfig
							.LOGD("[[tryToCleanDisk]] entry +++++++++++++++ current time = "
                                    + CommonUtilsRuntime.debugFormatTime(beginTime));
				}

				long diskSize = FileOperatorHelper.getDirectorySize(new File(
						fullPath));

				if (DEBUG) {
					CommonUtilsConfig.LOGD("          current disk Size = "
                            + FileUtil.convertStorage(diskSize)
                            + " max size = "
                            + FileUtil.convertStorage(maxSize)
                            + " current time = "
                            + CommonUtilsRuntime.debugFormatTime(System
                            .currentTimeMillis()));
				}

				if (diskSize >= maxSize) {
					CommonUtilsConfig
							.LOGD("          disk size > MaxSize, so begin clean the disk to 1.0 of MAX");
					LinkedList<FileInfo> fileList = FileOperatorHelper
							.getFileInfoUnderDir(new File(fullPath));
					if (DEBUG) {
						CommonUtilsConfig
								.LOGD("------------------------------------------------");
						CommonUtilsConfig
								.LOGD("  before sort list by time : file under "
                                        + fullPath + " list : ");
						if (fileList != null) {
							for (FileInfo info : fileList) {
								CommonUtilsConfig.LOGD("          Info : "
                                        + info.toString());
							}
						}
						CommonUtilsConfig
					...
					if (DEBUG) {
						CommonUtilsConfig
								.LOGD("-------------------============------------------");
						CommonUtilsConfig
								.LOGD("  after sort list by time : file under "
                                        + fullPath + " list : ");
						for (FileInfo info : fileList) {
							CommonUtilsConfig.LOGD("          Info : "
                                    + info.toString());
					...
					CommonUtilsConfig.LOGD(" should delete size = "
                            + FileUtil.convertStorage(cleanSize));
					...
					for (FileInfo info : fileList) {
						if (!info.isDir) {
							...
							if (CommonUtilsConfig.UTILS_DEBUG) {
								CommonUtilsConfig.LOGD("    add file [["
                                        + info.toString()
                                        + "]] into delete list. delete Size = "
					...
					if (DEBUG) {
						CommonUtilsConfig.LOGD(" curSize = "
                                + FileUtil.convertStorage(curSize));
					}
					...
					if (DEBUG) {
						CommonUtilsConfig
								.LOGD("<><><><><><><><><><><><><><><><><><><><><><>");
						CommonUtilsConfig
								.LOGD(" should clean file info : file under "
                                        + fullPath + " list : ");
					...
					for (FileInfo info : cleanFileInfo) {
						CommonUtilsConfig.LOGD("        delete file : "
                                + info.toString());
						FileOperatorHelper.DeleteFile(info);
					}
				}
				...
				if (DEBUG) {
					CommonUtilsConfig
							.LOGD("[[tryToCleanDisk]] leave +++++++++++++++ current time = "
                                    + CommonUtilsRuntime.debugFormatTime(System
                                    .currentTimeMillis())
                                    + " begin time = "
                                    + CommonUtilsRuntime.debugFormatTime(beginTime));
	...
	public static void tryToCleanDisk() {
		...
		tryToCleanDiskUnderDir(CommonUtilsConfig.DEVICE_INFO.flashDataPath,
				MAX_FLASH_SIZE);
	}
