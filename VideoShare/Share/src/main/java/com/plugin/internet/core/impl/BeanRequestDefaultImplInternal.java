...
class BeanRequestDefaultImplInternal implements BeanRequestInterface {
    ...
    public <T> T request(RequestBase<T> request) throws NetWorkException {
        ...
        if (DEBUG) {
            CommonUtilsConfig.LOGD("Entery Internet request, current time = " + entryTime + "ms from 1970");
        }
        ...
        if (contentType.equals(RequestEntity.REQUEST_CONTENT_TYPE_TEXT_PLAIN)) {
            if (CommonUtilsConfig.DEBUG_NETWORK_ST) {
                if (baseParams != null) {
                    for (String key : baseParams.keySet()) {
        ...
        if (DEBUG) {
            CommonUtilsConfig.LOGD("before get internet data from server, time cost from entry = "
...
//            UtilsConfig.LOGD(response);
            ...
            CommonUtilsConfig.LOGD(sb.toString());
            sb.setLength(0);
            if (response != null) {
                try {
                    sb.append("| " + CommonUtilsConfig.jsonFormatter(response) + " \n");
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
            ...
            do {
                if (index >= sb.length()) {
                    ...
                    if ((index + step) < sb.length()) {
                        CommonUtilsConfig.LOGD(sb.substring(index, index + step), false);
                    } else {
                        CommonUtilsConfig.LOGD(sb.substring(index, sb.length()), false);
                    }
                }
                index = index + step;
...
//        try {
        ...
        if (!request.getHandleErrorSelf()) {
            ...
            if (errorResponse != null && errorResponse.errorCode > 0 && !TextUtils.isEmpty(errorResponse.errorMsg)) {
                if (errorResponse.errorCode >= InternetConfig.SYSTEM_ERROR_CODE_START
                        && errorResponse.errorCode <= InternetConfig.SYSTEM_ERROR_CODE_END) {
                    if (!ignore) {
                        if (mHttpHookListener != null) {
                    ...
                    businessErrorCode = errorResponse.errorCode;
                    businessErrorMsg = errorResponse.errorMsg;
                }
            }
        }
        ...
        if (ret != null && ret instanceof ResponseBase) {
            ResponseBase responseBase = (ResponseBase) ret;
            if (isBusinessError) {
                responseBase.businessErrorCode = businessErrorCode;
                responseBase.businessErrorMsg = businessErrorMsg;
            } else {
                responseBase.businessErrorCode = 0;
                responseBase.businessErrorMsg = null;
            }
        }

        if (DEBUG) {
            CommonUtilsConfig.LOGD("Before return, after success get the data from server, parse cost time from entry = "
